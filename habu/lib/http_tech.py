...
def http_tech(url, cache=True, verbose=False):
    ...
    for app in apps:
        ...
        for header in apps[app].get("headers", []):
            if header in r.headers:
                ...
                if "\;version:\\" in apps[app]["headers"][header]:
                    version_group = apps[app]["headers"][header].split("\;version:\\")[
                        1
                    ]

                try:
                    match = re.search(
                        header_regex, r.headers[header], flags=re.IGNORECASE
                    )
                except Exception:
                    continue
                ...
                if match or not header_regex:
                    ...
                    logging.info(
                        "{app} detected by {header} HTTP header = {header_content}".format(
                            app=app, header=header, header_content=r.headers[header]
                        )
                    )
                    if app not in tech:
                        tech[app] = apps[app]
        ...
        for key in ["script", "html"]:
            ...
            for item in apps[app].get(key, []):
                ...
                try:
                    match = re.search(
                        item_regex, r.text, flags=re.IGNORECASE & re.MULTILINE
                    )
                except Exception:
                    continue
                ...
                if match:
                    logging.info(
                        "{app} detected by HTML body with regex {regex}".format(
        ...
        for url_regex in apps[app].get("url", []):
            ...
            if match:
                logging.info(
                    "{app} detected by URL with regex {regex}".format(
        ...
        for meta in apps[app].get("meta", []):
            ...
            for tag in soup.find_all("meta", attrs={"name": meta}):
                ...
                if match:
                    logging.info(
                        "{app} detected by meta {meta} tag with regex {regex}".format(
                            app=app, meta=meta, regex=meta_regex
                        )
                    )
    ...
    for t in list(tech.keys()):
        for imply in tech[t].get("implies", []):
            imply = imply.split("\\;")[0]
            logging.info(
                "{imply} detected because implied by {t}".format(imply=imply, t=t)
            )
...
if __name__ == "__main__":
    print(
        json.dumps(
            http_tech("https://woocommerce.com", verbose=True, cache=True), indent=4
        )
    )
