...
@click.option(
    "-c", "count", default=5, help="How many packets to send/receive (default: 5)"
)
@click.option("-i", "iface", default=None, help="Interface to use")
@click.option(
    "-g", "graph", is_flag=True, default=False, help="Graph (requires matplotlib)"
)
...
def cmd_tcp_isn(ip, port, count, iface, graph, verbose):
    ...
    for _ in range(count):
        ...
        if ans:
            send(
                IP(dst=ip)
                / TCP(sport=pkt[TCP].sport, dport=port, ack=ans[TCP].seq + 1, flags="A")
            )
