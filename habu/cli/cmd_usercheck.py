...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Open each valid url in a webbrowser",
)
def cmd_usercheck(username, no_cache, verbose, wopen):
    ...
    for site, url in urls.items():
        ...
        if r.status_code == 200:
            if (
                requests.head(
                    url.format("zei4fee3q9"), allow_redirects=False
                ).status_code
                == 200
            ):
                logging.error(
                    "Received status 200 for user zei4fee3q9, maybe, the check needs to be fixed"
                )
            else:
                existent[site] = u
                if wopen:
